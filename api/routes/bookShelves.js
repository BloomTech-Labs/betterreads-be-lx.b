...
// EDIT A BOOKSHELF
router.put('/:bookshelfId', [checkForBookshelf, createBookshelfRequirements], editBookShelf)
...
async function getABookShelf (req, res) {
    try {
        req.bookshelf.books = await UserShelfBook.findBooksByShelfId(req.bookshelf.id)
        return res.status(200).json({status: "Success", bookshelf: req.bookshelf})
    } catch (error) {
        console.log(error)
        return res.status(500).json({status: "Failure", error: "Server Error"})
    }
...
async function editBookShelf (req, res) {
    try {
        ...
        req.bookshelf.private = req.body.private !== null ? req.body.private : req.bookshelf.private;
