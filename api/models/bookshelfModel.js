...
const update = async (body) => {
    const [res] = await db('bookshelfs').update(body).where({id: body.id}).returning('*')
    return res;
}
...
module.exports = {findAllBookshelfsByUserId,findByName,getAllBooksOfABookShelf,insert,findById,remove, update}